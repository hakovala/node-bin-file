 ...
 */
File.open = function(filepath, options, cb) {
 ...
 */
File.prototype.open = function(options, cb) {
	cb = makeCallback(arguments, this);
	if (typeof options === 'function')
		options = undefined;

	options = options || {};
	options.flags = options.flags || this._flags || 'r+';
	options.mode = options.mode || this._mode;

	debug('open: %s (%o)', this.filepath, options);

	fs.open(this.filepath, options.flags, options.mode, (err, fd) => {
		if (err) {
			if (err.code === 'ENOENT' && options.create) {
				options.create = false; // prevent recursive hell
				options.flags = 'w+';
				return this.open(options, cb);
			} else {
				return cb(err);
			}
		}
